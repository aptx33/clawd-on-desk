...
| `src/quota.js` | AI 额度监控数据层：CPA 代理调用、Cursor 本地 token 读取、Antigravity 模型分组聚合、Gemini CLI bucket 解析 |
| `src/quota-panel.js` | 额度面板窗口管理：hover 弹出/隐藏计时、tray 点击弹出、定位逻辑 |
| `src/quota-panel.html` | 额度面板 UI：进度条渲染、toggle 开关、light/dark 主题、动态高度上报 |
| `src/preload-quota.js` | 额度面板 contextBridge（quotaAPI：数据接收、刷新、高度上报、鼠标追踪） |
...
### AI 额度监控面板（quota panel）

鼠标悬停桌宠或左键点击系统托盘时弹出额度面板，实时显示各 AI 服务的用量和剩余配额。

**数据源**：
- **Cursor Pro**：从本地 SQLite 读取 accessToken → 调用 `api2.cursor.sh/auth/usage` 获取本月用量
- **Codex（ChatGPT）**：通过 CPA management API 代理调用 `chatgpt.com/backend-api/wham/usage`，获取 5 小时限额和周限额
- **Antigravity**：通过 CPA 代理调用 `daily-cloudcode-pa.googleapis.com/v1internal:fetchAvailableModels`，按 Claude / GPT / Gemini Pro / Gemini Flash 分组聚合（取最差剩余比例 + 最早重置时间）
- **Gemini CLI**：通过 CPA 代理调用 `cloudcode-pa.googleapis.com/v1internal:retrieveUserQuota`，按 bucket 白名单过滤

**敏感配置**：
- 项目根目录 `quota-secrets.json`（已 gitignore）存放 `cpaKey` 和 `cursorClientId`
- 代码中不硬编码任何 key，运行时优先读 secrets 文件，fallback 到 `clawd-prefs.json`

**核心机制**：
- 面板分为 hover 模式（unfocusable，鼠标离开自动关闭）和 tray 模式（focusable，blur 关闭）
- 顶部 toggle chip 控制各数据源显示/隐藏，偏好持久化
- 定时轮询（默认 5 分钟）+ 手动刷新按钮
- 宠物隐藏、mini mode、权限气泡弹出时不触发 hover
